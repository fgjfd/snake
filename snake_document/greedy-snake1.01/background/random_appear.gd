...
func _process(delta):
	...
	# 检查阶段切换
	if phase == 1 and game_time >= 60.0:
		phase = 2
		trap_interval = 3  # 第二分钟改为3秒
		spawn_timer.wait_time = trap_interval
		spawn_timer.start()
...
func _on_spawn_timer_timeout():
	if phase == 1:
		# 第一阶段：随机生成一些陷阱
		spawn_random_traps(40)  # 生成10个陷阱
	else:
		# 第二阶段：生成更多陷阱
		spawn_random_traps(40)  # 生成15个陷阱
...
func spawn_random_traps(count):
	...
	if trap_nodes.size() > 0:
		...
		print("激活了 %d 个随机陷阱" % trap_nodes.size())
...
func _update_active_traps(delta):
	...
	for idx in traps_to_remove:
		...
		# 取消陷阱逻辑状态
		if trap.type == "random":
			# 对于随机陷阱，我们不需要更新traps数组
			print("移除了 %d 个随机陷阱" % trap.nodes.size())
